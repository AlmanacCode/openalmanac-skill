...
`search_web` is a discriminated union on `source` and requires it explicitly — the old `{query, limit}` shape no longer validates. For fact-checking, call `search_web({ source: "web", query, limit? })`. If the article is about a community, subculture, or lived-experience topic, you can also use `search_web({ source: "reddit", subreddit?, query?, sort?, time_range?, limit? })` to check whether the community's own account contradicts the article; Reddit results include post score, flair, and num_comments and are most useful as a tie-breaker against promotional or one-sided secondary sources, not as a primary authority.
